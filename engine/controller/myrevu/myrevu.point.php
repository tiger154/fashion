...
$cnt = $CLASS_POINT->getPointCount($DB, Module::$param[1], $_SESSION['userno']);
$list = $CLASS_POINT->getPointList($DB, $CLASS_PAGE->getLimit(), Module::$param[1], $_SESSION['userno']);
for($i=0; $i<sizeof($list); $i++) {
	$list[$i]['sign'] = ($list[$i]['flag_sign'] == "0") ? "+" : "-";
	$list[$i]['span_class'] = ($list[$i]['flag_sign'] == "0") ? "red_b_text" : "gray_lb_text";
	$_pointInfo = $CLASS_CODE->getPointType($DB, $list[$i]['point_type']);
	$list[$i]['point_explain'] = $_pointInfo['state_explain'];	
	if(!empty($list[$i]['rno'])) {
