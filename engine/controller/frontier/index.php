...
$toplist		= $CLASS_FRONTIER->FrontierListSelectT($DB, $code, $flag_delete="0", $btitle, $sch_opt, $sch_con, "B", $frname, "T"); 

$mainlist	= $CLASS_FRONTIER->FrontierListSelectM($DB, $code, $flag_delete="0", $btitle, $sch_opt, $sch_con, "B", $frname);

$frontierCount = $CLASS_FRONTIER->getListCount($DB, $code, $flag_delete="0", $btitle, $sch_opt, $sch_con, "B", $frname);
...
for($i=0;$i<sizeof($mainlist);$i++) {
	...
	$mainlist[$i]['frproduct'] = $CLASS_BASE->strLimitUTF($mainlist[$i]['frproduct'], "24", "", $eStr="...");
