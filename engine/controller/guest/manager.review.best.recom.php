...
$cnt = $CLASS_REVIEW->getBestWeekCount($DB);
$list = $CLASS_REVIEW->getBestWeekList($DB, $limit);
$size = sizeof($list);
...
$wno = $CLASS_REVIEW->getBestWeekWNo($DB, date("Y-m-d")); // 후보선정주
$wno_prev = $CLASS_REVIEW->getBestWeekPrev($DB, $wno); // 투표주
for($i=0; $i<$size; $i++) {
}
