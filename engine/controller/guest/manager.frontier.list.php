...
// 게시물카운트
$cnt = $CLASS_FRONTIER->getListCount($DB, $code, $flag_delete="0", $btitle, $sch_opt, $sch_con, $stat, $frname);

$CLASS_PAGE->set('totalRowCount', $cnt);

// 게시물리스트
$list = $CLASS_FRONTIER->getList($DB, $code, $btitle, $sch_opt, $sch_con, $CLASS_PAGE->getLimit(), $stat, $frname);
...
for($i=0;$i<sizeof($list);$i++) {
	...
	if(strtotime($list[$i]['start_date']) < $today and $today < strtotime($list[$i]['end_date'])){
			...
			$list[$i]['fronstat']= "<font color=#336633>모집중</font>";

	}else if(strtotime($list[$i]['end_date']) < $today){

			if(strtotime($list[$i]['notice_date']) > $today){
				$list[$i]['fronstat'] = "<font color=#996600>당첨자 선정중</font>";
			}else if(strtotime($list[$i]['due_edate']) < $today){
				$list[$i]['fronstat']= "<font color=#FF0000>종료</font>";
			}else{
				$list[$i]['fronstat']= "<font color=#6633FF>리뷰중</font>";
			}
	...
	if($list[$i]['maindisplay'] == "1"){
		$list[$i]['maindisplay'] = "<b><a href='javascript:frontierMaindisplay(\"".$list[$i]['frno']."\", \"\");'><font color=red>Y</a></b>";
	}else{
		$list[$i]['maindisplay'] = "<a href='javascript:frontierMaindisplay(\"".$list[$i]['frno']."\", \"1\");'>N</a>";
	}
