...
define('MAX_FILE_SIZE', 6000000); // Extended len
define('REAL_MAX_FILE_SIZE', 600000); // Extended len
// helper functions
// -----------------------------------------------------------------------------
...
function file_get_data_htmldom($url) {
			$data = array();
			$ch = curl_init();
			...
			curl_setopt($ch, CURLOPT_CONNECTTIMEOUT ,1);  // Connect time - timeout in seconds
			curl_setopt($ch, CURLOPT_TIMEOUT, 15); // Download time - timeout in seconds
...
function file_get_html_curl($url, $use_include_path = false, $context=null, $offset = -1, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
{
    // We DO force the tags to be terminated.
    $dom = new simple_html_dom(null, $lowercase, $forceTagsClosed, $target_charset, $stripRN, $defaultBRText, $defaultSpanText);
    // For sourceforge users: uncomment the next line and comment the retreive_url_contents line 2 lines down if it is not already done.
    //$contents = file_get_contents($url, $use_include_path, $context, $offset);
	$contents = file_get_data_htmldom($url);
    // Paperg - use our own mechanism for getting the contents as we want to control the timeout.
    //$contents = retrieve_url_contents($url);
    if (empty($contents) || strlen($contents) > MAX_FILE_SIZE)
    {
	...
	//**@@ Added by Revu 2012.11.06 for management of memory leak
	if(strlen($contents)> REAL_MAX_FILE_SIZE){
		$contents = substr($contents,0,REAL_MAX_FILE_SIZE);	
	}
