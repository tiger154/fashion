...
_editor_url = "/extends/editor.htmlarea/";
_editor_lang = "kr";
...
(function()
{
	if(typeof _editor_url =="string") {
		...
		alert("경고 : _editor_url 변수가 설정되지 않았습니다. 이 변수에 경로를 설정하세요. '/htmlarea/' 처럼 절대경로로 설정하세요.");
		_editor_url='';
	}	
...
HTMLArea.loadScript = function(url,plugin) {
	if(plugin) url = HTMLArea.getPluginDir(plugin) + '/' + url;
...
HTMLArea.init=function() { 
	...
	function loadNextScript()
	{
		...
		if(current<HTMLArea._scripts.length) {
			...
			document.title = "[HTMLArea: 스크립트 로딩 " + current + "/" + HTMLArea._scripts.length + "]";
...
// 설정 스크립트 로드
HTMLArea.loadScript(_editor_url+"config/dialog.js");
HTMLArea.loadScript(_editor_url+"config/popupwin.js");
HTMLArea.loadScript(_editor_url+"config/colors.js");
...
HTMLArea.Config = function()
{
	...
	*/
	...
	[	
		...
		["fontname", "space", "fontsize", "space", "formatblock", "separator", 
...
HTMLArea.Config.prototype.registerButton = function(id,tooltip,image,textMode,action,context)
{
	...
	if(typeof id=="string") {
		...
		alert("에러 [HTMLArea.Config::registerButton]:\n유효하지 않은 변수[버튼]입니다.");
		return false;
	}	
...
HTMLArea.prototype._createToolbar = function()
{
	...
	if(HTMLArea.is_ie) toolbar.style.width="650";
	...
	function createButton(txt){
		...
		if(!el&&btn){
			...
			HTMLArea._addEvent(el,"mouseout",function(){if(obj.enabled)with(HTMLArea){_removeClass(el,"buttonHover");_removeClass(el,"buttonActive");(obj.active)&&_addClass(el,"buttonPressed");}});
			HTMLArea._addEvent(el,"mousedown",function(ev){if(obj.enabled)with(HTMLArea){_addClass(el,"buttonActive");_removeClass(el,"buttonPressed");_stopEvent(is_ie?window.event:ev);}});
			HTMLArea._addEvent(el,"click",function(ev){if(obj.enabled)with(HTMLArea){_removeClass(el,"buttonActive");_removeClass(el,"buttonHover");obj.cmd(editor,obj.name,obj);_stopEvent(is_ie?window.event:ev);}});
			var img=document.createElement("img");
			img.src=btn[1];
			img.style.width="18px";
			img.style.height="18px";
			if(HTMLArea.is_ie)img.ondragstart=HTMLArea.do_false;el.appendChild(img);
		}else if(!el){
			el=createSelect(txt);
		}
		if(el){
...
HTMLArea.prototype.generate = function()
{
	...
	function initIframe() {
		...
		if(!editor.config.fullPage) {
			...
			html += "<link rel='stylesheet' href='"+_editor_url+"css/iframe.css' />";
...
HTMLArea.loadStyle(typeof _editor_css=="string"?_editor_css:"css/htmlarea.css");
...
HTMLArea.prototype.debugTree = function()
{
	...
	function _dt(root,level)
	{
		...
		var ns = HTMLArea.is_ie?root.scopeName:root.prefix;debug(level,"- "+tag+" ["+ns+"]");		
...
HTMLArea.prototype._wordClean = function(want_stats)
{
	...
	{
		if(HTMLArea.is_ie) el.outerHTML = HTMLArea.htmlEncode(el.innerText);
		else
		{
...
HTMLArea.prototype.toggleTableBorders = function()
{
	...
	if(!css)
	{
		...
		css.media = "all";
		css.rel = "stylesheet";
		css.href = _editor_url + 'css/table-borders.css';
...
HTMLArea.prototype._editorEvent = function(ev)
{
	...
	if(keyEvent&&ev.ctrlKey&&!ev.altKey) {
		...
		switch(key)
		{
			...
			case 'n': cmd="formatblock"; value="p"; break;
			...
			case '4':
			case '5':
			case '6': cmd="formatblock"; value="h"+key; break;
		}		
		if(cmd) {
		...
		switch(ev.keyCode)
		{
			case 13:
				if(HTMLArea.is_gecko&&!ev.shiftKey)
					if(this.dom_checkInsertP(true)) HTMLArea._stopEvent(ev);
				break;			
			case 8:
			case 46:
				if(HTMLArea.is_gecko&&!ev.shiftKey) {
	...
	editor._timerToolbar=setTimeout(function(){editor.updateToolbar();editor._timerToolbar=null;},50);
};
...
HTMLArea.getHTML = function(root,outputRoot,editor)
{
	...
	catch(e) { alert(e); return editor._iframe.contentWindow.document.body.innerHTML; }
};
