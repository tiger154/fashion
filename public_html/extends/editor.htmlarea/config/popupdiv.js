...
PopupDiv.prototype.defaultSize = function()
{
	...
	var addX = (is_ie&&is_compat)?(parseInt(cs.borderLeftWidth) + parseInt(cs.borderRightWidth) + parseInt(cs.paddingLeft) + parseInt(cs.paddingRight)):0;
	var addY = (is_ie&&is_compat)?(parseInt(cs.borderTopWidth) + parseInt(cs.borderBottomWidth) + parseInt(cs.paddingTop) + parseInt(cs.paddingBottom)):0;
	s.display = "block";
...
PopupDiv.prototype.callHandler = function()
{
	...
	for(var ti = tags.length;--ti>=0;)
	{
		...
		{
			var el=els[j];params[el.name]=el.value;
		}
	}
