...
/* 추가 현재 선택한 아이템의 순서정보를 가져옴 */
dhtmlXTreeObject.prototype.getOrder=function(itemId){	
	...
	{
		if(temp.childNodes[i].id == itemId) {
			order = i;
		}
	}
	return order;
...
/* 추가 현재 선택한 아이템의 같은 레벨의 순서정보를 가져옴 */
dhtmlXTreeObject.prototype.getOrderItem=function(itemId){	
	parentId = this.getParentId(itemId);
	var temp=this._globalIdStorageFind(parentId);
	if(!temp)return 0;
	var orderInfo = "";
	...
	{
		if(i == 0) {
			orderInfo = orderInfo + i + "|" + temp.childNodes[i].id;
		} else {
			orderInfo = orderInfo + "," +  i + "|" + temp.childNodes[i].id;
		}
	}
...
dhtmlXTreeObject.prototype.getAllItems =function(itemId)
{
	return this._getAllItems(itemId);
}

dhtmlXTreeObject.prototype._getAllItems =function(itemId,z,node)
{
	if(node) {
		temp=node;
	} else {
	...
	for(var i=0;i<temp.childsCount;i++)
	{
		level = this.getLevel(temp.childNodes[i].id);
		order = i;		
		parentId = this.getParentId(temp.childNodes[i].id);
		if(!z) {
			//z = 현재아이디 + 부모아이디 + 뎁스(레벨) + 순서
			z = temp.childNodes[i].id + "|" + parentId + "|" + level + "|" + order;
		} else {			
			z += "," + temp.childNodes[i].id + "|" + parentId + "|" + level + "|" + order;
		}
		var zb=this._getAllItems(0,z,temp.childNodes[i])
		if(zb) {
			z+=","+zb;
		}
	}
